  ...
  return <div className="clinical-page-shell clinical-page-shell--narrow min-h-screen bg-background">
    ...
    <div className="mt-5 grid gap-3 sm:grid-cols-2">
      {areas.map((area) => <article key={area.code} className="rounded-2xl border bg-card p-4">
        <div className="flex items-center justify-between gap-3"><div><p className="text-xs font-black text-blue-700">{area.code}</p><h2 className="font-bold">{area.title_full || area.title || "Specification area"}</h2></div><span className={`rounded-full px-2 py-1 text-xs font-bold ${area.band.tone === "emerald" ? "bg-emerald-50 text-emerald-700" : area.band.tone === "amber" ? "bg-amber-50 text-amber-700" : "bg-rose-50 text-rose-700"}`}>{area.score}% · {area.band.label}</span></div>
        <div className="mt-3 h-2 overflow-hidden rounded-full bg-muted"><div className="h-full bg-blue-600" style={{ width: `${area.score}%` }}/></div>
