    ...
    <div className="mx-auto max-w-7xl">
      ...
      <div className="mt-5 grid gap-5 lg:grid-cols-[340px_1fr]">
        <aside className="rounded-[22px] border border-white bg-white p-4 shadow-sm">
          <label className="relative block"><Search className="absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-slate-400" /><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Search learners…" className="w-full rounded-xl border border-slate-200 py-3 pl-10 pr-3 text-sm outline-none focus:border-violet-500" /></label>
        ...
        <section>
          {!selected ? <div className="rounded-[22px] border border-white bg-white p-10 text-center shadow-sm"><ClipboardCheck className="mx-auto h-9 w-9 text-slate-300" /><p className="mt-3 text-sm font-bold text-slate-500">Select a learner portfolio.</p></div> : <>
            <div className="rounded-[22px] border border-white bg-white p-5 shadow-sm"><div className="flex flex-wrap items-start justify-between gap-3"><div><p className="text-xs font-black uppercase tracking-wide text-violet-700">Connected case · {selected.case_name}</p><h2 className="mt-1 text-2xl font-black text-slate-950">{selected.student_name || "Learner"}</h2><p className="mt-1 text-xs text-slate-500">Last active: {selected.active_task?.replace("-", " ")} · {selected.active_section?.replaceAll("-", " ")}</p></div><p className="text-3xl font-black text-emerald-700">{Object.values(progress).filter(Boolean).length}/12</p></div></div>
