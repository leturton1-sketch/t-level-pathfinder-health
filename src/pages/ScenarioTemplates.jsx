    ...
    return (
      <div className="min-h-screen bg-clinical-navy flex items-center justify-center px-4">
        <div className="max-w-sm w-full rounded-2xl bg-white border border-clinical-red/30 shadow-xl p-6 text-center">
          ...
          <p className="text-sm text-slate-500 mb-4">Template authoring is only available to tutors, admins, and super admins.</p>
    ...
    <div className="min-h-screen bg-clinical-navy pb-24">
        ...
        <div className="max-w-4xl mx-auto px-3 py-2.5 sm:px-4">
          <div className="flex items-center justify-between gap-2">
            <div className="flex items-center gap-2 min-w-0">
              ...
              <h1 className="font-heading font-bold text-sm sm:text-base text-slate-800 truncate">Patient Scenario Templates</h1>
      ...
      <div className="max-w-4xl mx-auto px-3 sm:px-4 pt-4">
        ...
        <div className="rounded-xl border border-clinical-teal/30 bg-clinical-teal/5 p-3.5 mb-4 flex items-start gap-3">
          ...
          <div>
            <h2 className="text-sm font-heading font-bold text-slate-800 mb-0.5">Reusable Template Library</h2>
            <p className="text-xs text-slate-600 leading-relaxed">Build patient scenarios once, save them as templates, then clone and adapt them for new simulation exercises. Use a template directly in the ward simulator with one tap.</p>
          </div>
        </div>
        ...
        {loading ? (
          ...
          <div className="rounded-xl border border-dashed border-slate-300 bg-white/50 p-8 text-center">
            ...
            <p className="text-sm font-heading font-semibold text-slate-700 mb-1">No templates yet</p>
            <p className="text-xs text-slate-500 mb-4">Create a reusable patient scenario template to get started.</p>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-2.5">
            {templates.map((t) => (
              <div key={t.id} className="rounded-xl border border-slate-200 bg-white p-3.5 shadow-sm flex flex-col">
                <div className="flex items-start justify-between gap-2 mb-2">
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-1.5 mb-1">
                      ...
                      <span className="flex items-center gap-1 text-[10px] text-slate-400"><Clock className="w-3 h-3" /> {t.estimated_duration} min</span>
                      {t.category && <span className="text-[10px] text-slate-400 capitalize">{t.category.replace("_", " ")}</span>}
                    </div>
                    <h3 className="font-heading font-bold text-sm text-slate-800 truncate">{t.name}</h3>
                    {t.description && <p className="text-xs text-slate-500 mt-0.5 leading-snug line-clamp-2">{t.description}</p>}
                  </div>
                ...
                <div className="flex items-center gap-1 mt-auto pt-1">
                  ...
                  <button onClick={() => handleClone(t)} className="flex items-center gap-1 rounded-md border border-slate-300 bg-white px-2 py-1.5 text-[10px] font-heading font-semibold text-slate-600 hover:bg-slate-50" title="Clone to new template">
                    <Copy className="w-3 h-3" />
                  </button>
                  <button onClick={() => handleEdit(t)} className="flex items-center gap-1 rounded-md border border-slate-300 bg-white px-2 py-1.5 text-[10px] font-heading font-semibold text-slate-600 hover:bg-slate-50" title="Edit template">
                    <Pencil className="w-3 h-3" />
                  </button>
                  <button onClick={() => setConfirmDelete(t)} className="flex items-center gap-1 rounded-md border border-clinical-red/30 bg-white px-2 py-1.5 text-[10px] font-heading font-semibold text-clinical-red hover:bg-clinical-red/5" title="Delete template">
        ...
        <div className="fixed inset-0 z-[60] flex items-center justify-center bg-black/30 backdrop-blur-sm animate-fade-in" onClick={() => setConfirmDelete(null)}>
          <div className="bg-white rounded-xl shadow-2xl p-6 max-w-sm w-[90%]" onClick={(e) => e.stopPropagation()}>
            <div className="flex items-center gap-3 mb-3">
              ...
              <h2 className="font-heading font-bold text-slate-800">Delete Template?</h2>
            </div>
            <p className="text-sm text-slate-600 mb-4">Delete "{confirmDelete.name}"? Scenarios already created from it are unaffected.</p>
