...
import { User, BookOpen, ClipboardList, Stethoscope, LogOut, BarChart3, Target, TrendingUp } from "lucide-react";
...
export default function Profile() {
  ...
  const loadData = async () => {
    try {
      const r = await base44.entities.SimulationResult.filter({ student_id: user.id });
      setResults(r);
    } catch {}
    try {
      const s = await base44.entities.CarePlanSubmission.filter({ student_id: user.id });
      setSubmissions(s);
    } catch {}
  ...
  const avgSimScore = results.length > 0
    ...
    <div className="min-h-screen bg-clinical-navy px-4 pt-6 pb-24 max-w-3xl mx-auto">
      ...
      <div className="flex items-center gap-4 mb-6">
        ...
        <button
          ...
          className="p-2.5 rounded-lg border border-clinical-red/30 text-clinical-red hover:bg-clinical-red/10 transition-colors"
          title="Log out"
        >
      ...
      <div className="rounded-xl border border-border bg-card/60 p-4 mb-4">
        ...
        <div className="space-y-1.5">
          {Object.entries(SK_CODES).slice(0, 10).map(([code, desc]) => {
            const completed = Math.random() > 0.5; // Placeholder — would be calculated from actual results
            return (
        ...
        <div className="grid grid-cols-2 gap-3">
          <div>
            ...
            <div className="flex gap-2">
              <span className={`text-xs rounded-lg border px-3 py-1.5 ${user?.ai_persona === "female" ? "border-clinical-teal bg-clinical-teal/10 text-clinical-teal" : "border-border text-muted-foreground"}`}>
                Female
              </span>
              <span className={`text-xs rounded-lg border px-3 py-1.5 ${user?.ai_persona === "male" ? "border-clinical-teal bg-clinical-teal/10 text-clinical-teal" : "border-border text-muted-foreground"}`}>
                Male
              </span>
            </div>
          </div>
      ...
      <div className="rounded-xl border border-border bg-card/60 p-4">
        ...
        <div className="space-y-2">
          {results.slice(0, 3).map((r, i) => (
            <div key={i} className="flex items-center justify-between text-xs">
              <div className="flex items-center gap-2">
                <Stethoscope className="w-3.5 h-3.5 text-clinical-amber" />
                <span className="text-foreground">{r.scenario_name}</span>
              </div>
          ...
          {submissions.slice(0, 3).map((s, i) => (
            <div key={i} className="flex items-center justify-between text-xs">
              <div className="flex items-center gap-2">
                <ClipboardList className="w-3.5 h-3.5 text-clinical-green" />
                <span className="text-foreground">{s.title}</span>
              </div>
              <span className={`font-semibold ${s.status === "reviewed" ? "text-clinical-green" : "text-clinical-amber"}`}>
                {s.status}
              </span>
            </div>
          ))}
