...
export default function TalentCardPage() {
  ...
  return <div className="clinical-page-shell clinical-page-shell--narrow min-h-screen bg-background">
    <article className="overflow-hidden rounded-[28px] border bg-card shadow-sm">
      <div className="bg-gradient-to-r from-blue-700 to-sky-500 p-6 text-white"><p className="text-xs font-black tracking-[.18em]">PATHFINDER TALENT CARD</p><div className="mt-3 flex items-end justify-between gap-4"><div><h1 className="text-3xl font-black">{card.student_name}</h1><p className="mt-1 text-sm opacity-90">{card.career_aspiration || "Career aspiration to be confirmed"}</p></div>{card.verified && <BadgeCheck className="h-8 w-8"/>}</div></div>
