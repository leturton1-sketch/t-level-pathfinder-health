...
export default function ESPPortfolioReview() {
  ...
  return <main className="esp-surface clinical-page-shell min-h-screen bg-slate-50 pb-28 print:bg-white">
    <div className="mx-auto max-w-5xl">
      ...
      <div className="mt-5 space-y-5">
        {Object.entries(SECTION_NAMES).map(([key, title]) => {
          ...
          return <article key={key} className="break-inside-avoid rounded-[22px] border border-slate-200 bg-white p-5 shadow-sm print:rounded-none print:shadow-none">
            ...
            {hasEvidence ? <dl className="mt-4 space-y-4 border-t border-slate-100 pt-4">{Object.entries(values).map(([field, value]) => value?.trim() && <div key={field}><dt className="text-xs font-black text-sky-800">{label(field)}</dt><dd className="mt-1 whitespace-pre-wrap text-sm leading-6 text-slate-700">{value}</dd></div>)}</dl> : <p className="mt-4 rounded-xl bg-slate-50 p-4 text-xs text-slate-500">No evidence saved for this section.</p>}
          </article>;
        })}
      </div>

      <section className="mt-5 rounded-[22px] border border-slate-200 bg-white p-5 shadow-sm print:shadow-none">
        <div className="flex items-center gap-2"><FileText className="h-5 w-5 text-sky-700" /><h2 className="font-black text-slate-950">Tutor review</h2></div>
      ...
      <section className="mt-5 rounded-[22px] border border-sky-200 bg-sky-50 p-5 print:hidden">
        <div className="flex flex-wrap items-center justify-between gap-4">
          <div>
            <h2 className="font-black text-sky-950">Submit for tutor review</h2>
            <p className="mt-1 text-xs leading-5 text-sky-900">{isSubmitted ? "Your connected portfolio has been sent to the teaching team." : allComplete ? "All 12 sections are complete. You can now send this practice portfolio for feedback." : `Complete all 12 sections before submitting. ${12 - completed} remaining.`}</p>
          </div>
          <button disabled={!allComplete || isSubmitted} onClick={() => updatePortfolio({ status: "submitted" })} className="inline-flex min-h-11 items-center gap-2 rounded-xl bg-sky-700 px-4 text-sm font-black text-white disabled:cursor-not-allowed disabled:bg-slate-300">
