...
function Explorer() {
  ...
  <div className="grid gap-4 min-[1600px]:grid-cols-[260px_minmax(0,1fr)_320px]">
    <aside className={`${panel} p-4`}>
      <p className="mb-2 text-[10px] font-black uppercase tracking-[.18em] text-sky-700">Outer to inner layers</p>
    ...
    <section className={`${panel} overflow-hidden p-3`}>
      ...
      <div className="mb-3 flex flex-wrap gap-2 px-2" role="group" aria-label="Anatomical camera view">
        {[
          ...
          className={`flex items-center gap-1.5 rounded-xl border px-3 py-2 text-[11px] font-black transition ${viewMode === mode ? "border-sky-600 bg-sky-600 text-white shadow-md" : "border-slate-200 bg-white text-slate-700 hover:border-sky-300"}`}>
      ...
      {isAdminUser && (
        <button onClick={() => setShowAnimController((v) => !v)} className={`flex items-center gap-1.5 rounded-xl border px-3 py-2 text-[11px] font-black transition ${showAnimController ? "border-sky-500 bg-sky-600 text-white shadow-md" : "border-slate-200 bg-white text-slate-700 hover:border-sky-300"}`}>
    ...
    <aside className={`${panel} p-6`}>
      {selected ? <>
        ...
        <div className="mt-4 rounded-2xl bg-white/80 p-4"><p className="text-[10px] font-black uppercase tracking-wider text-sky-700">Structure & function</p><p className="mt-2 text-sm leading-6 text-slate-700">{selected.function}</p></div>
      ...
      </> : <div className="grid min-h-[400px] place-items-center text-center"><div><Info className="mx-auto h-10 w-10 text-sky-500"/><h2 className="mt-3 font-black text-slate-900">Select a body part</h2><p className="mt-2 text-sm text-slate-600">Click any visible structure to explore its anatomy, physiology and clinical relevance.</p></div></div>}
    </aside>
  </div>
  </>;
}
...
function Pathophysiology() {
  ...
  return <div className="grid gap-4 min-[1600px]:grid-cols-[300px_minmax(0,1fr)_340px]">
    ...
    <aside className={`${panel} p-6`}>
      ...
      <h3 className="mt-6 text-sm font-black text-slate-900">Expected presentation</h3><ul className="mt-2 space-y-2">{condition.signs.map((sign)=><li key={sign} className="flex items-center gap-2 text-sm text-slate-700"><ShieldAlert className="h-4 w-4 text-rose-500"/>{sign}</li>)}</ul>
      <h3 className="mt-6 text-sm font-black text-slate-900">Care priorities</h3><ol className="mt-2 space-y-2">{condition.priorities.map((priority,index)=><li key={priority} className="flex gap-2 text-sm text-slate-700"><span className="grid h-5 w-5 shrink-0 place-items-center rounded-full bg-sky-100 text-[10px] font-black text-sky-700">{index+1}</span>{priority}</li>)}</ol>
    </aside>
  </div>;
}
...
function ScenarioLab() {
  ...
  return <div className="grid gap-4 min-[1600px]:grid-cols-[300px_minmax(0,1fr)_360px]">
    <aside className={`${panel} p-4`}><p className="mb-3 text-[10px] font-black uppercase tracking-[.18em] text-sky-700">Standardised patients</p>{STANDARDISED_PATIENTS.map((item)=><button key={item.id} onClick={()=>reset(item.id)} className={`mb-2 w-full rounded-2xl border p-3 text-left ${patientId===item.id?"border-sky-300 bg-sky-50":"border-white bg-white/75"}`}><p className="font-black text-slate-900">{item.name}</p><p className="text-xs text-slate-500">{item.age} years · {PATHOPHYSIOLOGY_CONDITIONS.find((c)=>c.id===item.conditionId)?.name}</p></button>)}
    ...
    <section className={`${panel} p-6`}>
      <div className="flex items-start justify-between gap-4"><div><p className="text-[10px] font-black uppercase tracking-[.18em] text-sky-700">Clinical decision lab</p><h2 className="text-2xl font-black text-slate-900">{condition.name}</h2></div><ClipboardCheck className="h-8 w-8 text-sky-600"/></div>
      <h3 className="mt-6 text-sm font-black text-slate-900">1. Record the vital information</h3><p className="mt-1 text-xs text-slate-600">Enter your own measured values. All fields are required before a complete review can be awarded.</p>
      ...
      <h3 className="mt-6 text-sm font-black text-slate-900">2. Select and sequence care interventions</h3><div className="mt-3 space-y-2">{patient.choices.map((choice)=><button key={choice.id} onClick={()=>toggleChoice(choice.id)} className={`flex w-full items-center gap-3 rounded-2xl border p-3 text-left transition ${choices.includes(choice.id)?"border-sky-400 bg-sky-50":"border-slate-200 bg-white"}`}><span className={`grid h-7 w-7 place-items-center rounded-full ${choices.includes(choice.id)?"bg-sky-600 text-white":"bg-slate-100 text-slate-500"}`}>{choices.includes(choice.id)?<CheckCircle2 className="h-4 w-4"/>:<ChevronRight className="h-4 w-4"/>}</span><span className="text-sm font-bold text-slate-800">{choice.label}</span></button>)}</div>
      <button onClick={()=>setFeedback(calculateScenarioFeedback(patient,observations,choices))} className="mt-6 w-full rounded-2xl bg-gradient-to-r from-sky-600 to-cyan-600 px-5 py-3 text-sm font-black text-white shadow-lg"><Sparkles className="mr-2 inline h-4 w-4"/>Submit session for formative feedback</button>
    </section>
    <aside className={`${panel} p-6`}>
      {!feedback ? <div className="grid min-h-[480px] place-items-center text-center"><div><Activity className="mx-auto h-12 w-12 text-sky-500"/><h2 className="mt-3 font-black text-slate-900">Patient response monitor</h2><p className="mt-2 text-sm text-slate-600">Your observations and interventions determine whether the patient improves, stabilises or deteriorates.</p></div></div> : <>
...
export default function AnatomyPhysiology() {
  ...
  return <main className="clinical-page-shell min-h-screen bg-[radial-gradient(circle_at_10%_5%,rgba(255,255,255,.98),transparent_30%),radial-gradient(circle_at_88%_14%,rgba(205,190,235,.48),transparent_32%),linear-gradient(145deg,#fbfafc,#f0edf5_54%,#f8f7fa)] text-slate-900">
    <div className="mx-auto max-w-[1600px]">
      <header className={`${panel} mb-6 flex flex-wrap items-center justify-between gap-4 p-6`}><div className="flex items-center gap-4"><span className="grid h-14 w-14 place-items-center rounded-2xl bg-gradient-to-br from-sky-500 to-cyan-600 text-white shadow-xl"><Brain className="h-7 w-7"/></span><div><p className="text-[10px] font-black uppercase tracking-[.22em] text-sky-700">T Level Health · Areas 8–9</p><h1 className="text-2xl font-black tracking-tight sm:text-3xl">Anatomy, Physiology & Pathophysiology</h1><p className="mt-1 text-sm text-slate-600">Explore structures, visualise disease processes and practise care decisions safely.</p></div></div><span className="rounded-full bg-emerald-100 px-4 py-2 text-xs font-black text-emerald-800">Interactive learning module</span></header>
