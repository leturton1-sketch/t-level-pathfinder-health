    ...
    <div className="rounded-lg border border-purple-200 bg-purple-50/40 p-3 space-y-3">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-1.5">
          ...
          <span className="text-xs font-heading font-bold text-purple-900">Branching Pathways</span>
          <span className="text-[10px] text-slate-500">{Object.keys(tree.nodes).length} nodes · {totalOptions} choices</span>
        </div>
        <button onClick={addNode} className="flex items-center gap-1 rounded-md bg-purple-600 text-white px-2 py-1 text-[10px] font-heading font-semibold hover:opacity-90">
      ...
      <div className="space-y-2">
        {nodeOrder.map((nodeId, idx) => {
          ...
          return (
            <div key={nodeId} className={`rounded-xl border bg-white ${isEntry ? "border-purple-400 ring-1 ring-purple-300" : "border-slate-200"}`}>
              <div className="flex items-center gap-1.5 px-2.5 py-1.5 border-b border-slate-100">
                <GripVertical className="w-3 h-3 text-slate-300" />
                <span className={`grid h-5 w-5 place-items-center rounded-md text-[10px] font-black ${isEntry ? "bg-purple-600 text-white" : "bg-slate-200 text-slate-600"}`}>{idx + 1}</span>
                {isEntry ? (
                  <span className="flex items-center gap-1 text-[10px] font-bold text-purple-700"><Flag className="w-3 h-3" /> Entry node</span>
                ) : (
                  <button onClick={() => setEntry(nodeId)} className="text-[10px] font-semibold text-slate-400 hover:text-purple-600">Set as entry</button>
                )}
              ...
              {!isCollapsed && (
                <div className="p-2.5 space-y-2">
                  <textarea value={node.prompt} onChange={(e) => updateNode(nodeId, { prompt: e.target.value })}
                    ...
                    rows={2}
                    className="w-full rounded-lg border border-slate-200 bg-white px-2.5 py-2 text-xs text-slate-700 focus:outline-none focus:border-purple-400 resize-y" />
                  ...
                  <div className="space-y-1.5">
                      ...
                      <div key={opt.id} className={`rounded-lg border p-2 space-y-1.5 ${opt.correct ? "border-clinical-green/40 bg-clinical-green/5" : "border-slate-200 bg-slate-50/60"}`}>
                        <div className="flex items-center gap-1.5">
                          ...
                          <input value={opt.label} onChange={(e) => updateOption(nodeId, opt.id, { label: e.target.value })}
                            ...
                            className="flex-1 rounded-md border border-slate-200 bg-white px-2 py-1 text-[11px] text-slate-700 focus:outline-none focus:border-purple-400" />
                        ...
                        <input value={opt.feedback} onChange={(e) => updateOption(nodeId, opt.id, { feedback: e.target.value })}
                          ...
                          className="w-full rounded-md border border-slate-200 bg-white px-2 py-1 text-[11px] text-slate-600 focus:outline-none focus:border-purple-400" />
                        <div className="flex items-center gap-1.5">
                          ...
                          {opt.next && tree.nodes[opt.next] && (
                            <button onClick={() => setCollapsed((c) => ({ ...c, [opt.next]: false }))}
                              className="shrink-0 flex items-center gap-0.5 rounded-md bg-purple-100 text-purple-700 px-1.5 py-1 text-[9px] font-bold hover:bg-purple-200">
                    ...
                    <button onClick={() => addOption(nodeId)} className="flex items-center gap-1 rounded-md border border-dashed border-purple-300 text-purple-600 px-2 py-1 text-[10px] font-heading font-semibold hover:bg-purple-50">
      ...
      {nodeOrder.length === 0 && (
        <div className="rounded-lg border border-dashed border-slate-300 bg-white p-4 text-center">
          ...
          <button onClick={addNode} className="inline-flex items-center gap-1 rounded-md bg-purple-600 text-white px-3 py-1.5 text-xs font-heading font-semibold hover:opacity-90">
