...
import { getQuickNavForRole } from "@/lib/navItems";
import NavCatalogSheet from "./NavCatalogSheet";

export default function BottomNav() {
  const navigate = useNavigate();
  const location = useLocation();
  const user = getCurrentUser();
  const nav = getQuickNavForRole(user?.role);
        ...
        <nav className="clinical-glass-nav pointer-events-auto relative mx-auto max-w-md rounded-[22px] px-2 py-1.5 opacity-55 transition-[opacity,transform] duration-700 hover:opacity-95 hover:-translate-y-0.5 focus-within:opacity-100 focus-within:-translate-y-0.5">
          <div className="flex items-center justify-around">
            ...
            <button
              ...
              className="relative flex flex-col items-center gap-0.5 px-3 py-1.5 rounded-xl text-muted-foreground hover:text-foreground transition-all"
              aria-label="Browse all tools"
            >
              <LayoutGrid className="w-5 h-5" />
              <span className="text-[10px] font-heading font-medium">More</span>
            </button>
          </div>
        </nav>
      </div>
      <NavCatalogSheet open={catalogOpen} onOpenChange={setCatalogOpen} onNavigate={go} />
    </>
  );
}