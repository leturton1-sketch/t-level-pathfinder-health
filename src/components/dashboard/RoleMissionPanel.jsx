...
export default function RoleMissionPanel({ user }) {
  ...
  return <section className="mb-4 rounded-[24px] border border-blue-200 bg-gradient-to-r from-blue-50 via-white to-rose-50 p-4 shadow-sm">
    <div className="flex flex-wrap items-start justify-between gap-4">
      <div><p className="text-[10px] font-black uppercase tracking-[.18em] text-blue-700">{config.eyebrow}</p><h2 className="mt-1 text-xl font-black text-slate-950">{config.title}</h2><p className="mt-1 max-w-3xl text-sm text-slate-600">{config.message}</p></div>
      <div className="flex flex-wrap gap-1.5">{PATHFINDER_HUBS.map((hub) => <span key={hub} className="rounded-full border border-blue-100 bg-white px-2.5 py-1 text-[10px] font-black text-blue-700">{hub}</span>)}</div>
    </div>
    <div className="mt-4 grid gap-2 sm:grid-cols-2 xl:grid-cols-4">{config.actions.map(([label, path, Icon]) => <Link key={path} to={path} className="flex min-h-12 items-center gap-2 rounded-xl border border-white bg-white/90 px-3 py-2 text-sm font-bold text-slate-800 shadow-sm transition hover:-translate-y-0.5 hover:border-blue-200"><Icon className="h-4 w-4 text-blue-600" />{label}</Link>)}</div>
  </section>;
}
