...
export default function ADLScenario({ scenario, bedDesignations, onActiveBedChange, onClose }) {
  ...
  if (phase === "setup") return <div className="absolute inset-0 z-50 grid place-items-center bg-slate-950/30 p-4 backdrop-blur-sm">
    <div className="polished-glass-edge w-full max-w-lg rounded-[28px] border border-white/90 bg-white/95 p-5 shadow-2xl">
      <div className="flex items-start justify-between"><div><p className="text-[10px] font-black uppercase tracking-[.18em] text-purple-700">SK15 · Group simulation</p><h2 className="mt-1 text-xl font-black text-slate-900">Activities of Daily Living</h2><p className="mt-1 text-sm text-slate-600">Add every learner before activating the ward call bells.</p></div><button onClick={onClose} className="rounded-xl p-2 hover:bg-slate-100"><X className="h-4 w-4" /></button></div>
      <div className="mt-4 flex gap-2"><input value={nameInput} onChange={(e) => setNameInput(e.target.value)} onKeyDown={(e) => e.key === "Enter" && addLearner()} placeholder="Learner name" className="flex-1 rounded-xl border border-slate-200 px-3 py-2 text-sm" /><button onClick={addLearner} className="rounded-xl bg-purple-700 px-4 text-sm font-bold text-white">Add</button></div>
