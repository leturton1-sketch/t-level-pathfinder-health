...
export default function ADLScenario({ scenario, bedDesignations, onActiveBedChange, onClose }) {
  ...
  return <div className="absolute inset-x-3 bottom-3 z-50 sm:left-auto sm:w-[430px]"><div className="polished-glass-edge max-h-[78vh] overflow-y-auto rounded-[26px] border border-white/90 bg-white/95 p-4 shadow-2xl backdrop-blur-2xl">
    <div className="flex items-center justify-between"><div><p className="text-[10px] font-black uppercase tracking-[.16em] text-blue-700">ADL simulation · {completed.length} complete</p><h2 className="text-base font-black">{activeCall ? `Call bell · Bed ${activeCall.bed}` : "Ward monitoring"}</h2></div><button onClick={finish} className="rounded-xl border px-3 py-2 text-xs font-bold">Finish</button></div>
    {!activeCall ? <div className="mt-4 rounded-2xl bg-slate-100 p-5 text-center"><BellRing className="mx-auto h-8 w-8 text-slate-400" /><p className="mt-2 text-sm font-bold">Waiting for the next patient call</p><button onClick={triggerCall} className="mt-3 text-xs font-bold text-blue-700">Trigger next call now</button></div> : <>
